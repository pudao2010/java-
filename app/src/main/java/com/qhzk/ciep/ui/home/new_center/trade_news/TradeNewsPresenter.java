...
public class TradeNewsPresenter extends BasePresenter<TradeNewsView> {
    public int page = 1;
    public void getTradeNewsData(){
        mApi.getNews(1, 20, "industnews", new ResultSubscriber<List<NewEntity>>(this) {
            @Override
            public void onSuccess(List<NewEntity> list) {
                ...
                getMvpView().onTradeNewsResult(list);
            }
        });
    }

    public void loadMore() {
        mApi.getNews(++page, 20, "industnews", new ResultSubscriber<List<NewEntity>>(this) {
            @Override
            public void onSuccess(List<NewEntity> list) {
                if (list == null){
                    return;
