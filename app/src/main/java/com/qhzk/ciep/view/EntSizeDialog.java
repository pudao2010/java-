...
public class EntSizeDialog extends Dialog {

    private ArrayList<String> mEntsizes;
    private WheelView mWheelView;

    public EntSizeDialog(Context context, OnCountrySelectListener l) {
        this(context, R.style.Theme_Light_Dialog);
        mOnCountrySelectListener = l;
    }

    public EntSizeDialog(Context context, int themeResId) {
    ...
    private void init(Context context) {
        ...
        String[] entsize = context.getResources().getStringArray(R.array.scale);
        mEntsizes = new ArrayList<>();
        for (int i = 0; i < entsize.length; i++) {
            mEntsizes.add(entsize[i]);
        }
        mWheelView.setData(mEntsizes);
